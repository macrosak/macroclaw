...
import { Claude2, QueryParseError, QueryProcessError, QueryValidationError, type Schema } from "./claude";
...
function makeClaude(config?: { model?: string; systemPrompt?: string }) {
  return new Claude2({ workspace: TEST_WORKSPACE, ...config });
}

const textResult = { type: "text" } as const;

function objectResult<T>(parse: (data: unknown) => T): { type: "object"; schema: Schema<T> } {
  return { type: "object", schema: { parse } };
}
...
describe("Claude2", () => {
  describe("newSession", () => {
    ...
    it("returns parsed value for object resultType", async () => {
      ...
      const claude = makeClaude();
      const schema = objectResult((d: unknown) => d as { action: string; message: string });
  ...
  describe("resultType", () => {
    ...
    it("passes --json-schema for object resultType", async () => {
      ...
      const claude = makeClaude();
      const schema = objectResult((d) => d as { ok: boolean });
    ...
    it("validates structured output through schema.parse", async () => {
      ...
      const claude = makeClaude();
      const parseFn = mock((d: unknown) => d as { count: number });
      const schema = objectResult(parseFn);
      await claude.newSession("hi", schema).result;
      expect(parseFn).toHaveBeenCalledWith({ count: 5 });
    });
  });
  ...
  describe("errors", () => {
    ...
    it("rejects with QueryValidationError when schema.parse throws", async () => {
      ...
      try {
        ...
        expect((err as QueryValidationError).cause).toBe(parseError);
      }
    });
  });
