...
import { type ClaudeDeferredResult, ClaudeParseError, ClaudeProcessError, type ClaudeResult, isDeferred, runClaude } from "./claude";
...
describe("runClaude", () => {
  it("passes --session-id flag when given", async () => {
    ...
    const result = await runClaudeSync(opts());
  ...
  it("returns deferred result when process exceeds timeout", async () => {
    ...
    Bun.spawn = mock((() => proc) as any);
    try {
      const result = await runClaude(opts({ sessionId: "sid-7", timeoutMs: 60_000 }));
  ...
  it("returns structured_output from successful response", async () => {
    ...
    const result = await runClaudeSync(opts({ sessionFlag: "--resume", sessionId: "sid-8" }));
  ...
  it("returns null structuredOutput and result text when structured_output is missing", async () => {
    ...
    const result = await runClaudeSync(opts({ sessionFlag: "--resume", sessionId: "sid-9" }));
  ...
  it("returns empty sessionId when session_id is missing from envelope", async () => {
    ...
    mockSpawn({ stdout: envelope, exitCode: 0 });
    const result = await runClaudeSync(opts({ sessionFlag: "--resume", sessionId: "sid-9c" }));
    expect(result.sessionId).toBe("");
  });

  it("returns result from envelope", async () => {
    mockSpawn({ stdout: jsonResult({ action: "send" }), exitCode: 0 });
    const result = await runClaudeSync(opts({ sessionFlag: "--resume", sessionId: "sid-9b" }));
  ...
  it("throws ClaudeParseError when stdout is not valid JSON", async () => {
    ...
    try {
      await runClaudeSync(opts({ sessionFlag: "--resume", sessionId: "sid-10" }));
      expect.unreachable("should have thrown");
    } catch (err) {
  ...
  it("returns duration and cost from envelope", async () => {
    mockSpawn({ stdout: jsonResult({ action: "send" }), exitCode: 0 });
    const result = await runClaudeSync(opts({ sessionFlag: "--resume", sessionId: "sid-11" }));
  ...
  it("does not set timeout when timeoutMs is not provided", async () => {
    mockSpawn({ stdout: jsonResult({ action: "send" }), exitCode: 0 });
    const result = await runClaudeSync(opts({ sessionFlag: "--resume", sessionId: "sid-12" }));
